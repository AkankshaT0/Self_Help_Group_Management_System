...
class ContributionManagement:
    ...
    def create_ui(self):
        ...
        # Right side - Credits and Totals
        self.right_frame = tk.Frame(self.main_container, bg=self.bg_color, width=300)
        self.right_frame.pack(side='right', fill='y', padx=10, pady=10)

        # Left side content (your existing UI)
        self.canvas = tk.Canvas(self.left_frame, bg=self.bg_color, highlightthickness=0)
        ...
        tk.Label(self.scrollable_frame, text="Contribution Management", font=self.title_font,
                 bg=self.bg_color, fg=self.fg_color).grid(row=0, column=0, columnspan=4, pady=20, sticky='w')

        form_frame = tk.LabelFrame(self.scrollable_frame, text="Add/Edit Contribution", font=self.label_font,
        ...
        for i, field_data in enumerate(fields):
            ...
            if field_type == 'text':
                ...
                date_entry = DateEntry(form_frame, width=20, font=self.entry_font, background='darkblue', foreground='white', borderwidth=2)
                ...
                browse_btn = tk.Button(form_frame, text="Browse", command=lambda var=file_var: self.browse_file(var), font=self.entry_font)
        ...
        tk.Button(button_frame, text="Add Contribution", command=self.add_contribution, bg=self.fg_color, fg='white', **btn_cfg).pack(side='left', padx=10)
        tk.Button(button_frame, text="Update", command=self.update_contribution, bg=self.fg_color, fg='white', **btn_cfg).pack(side='left', padx=10)
        tk.Button(button_frame, text="Delete", command=self.delete_contribution, bg=self.fg_color, fg='white', **btn_cfg).pack(side='left', padx=10)
        tk.Button(button_frame, text="Generate PDF", command=self.generate_pdf, bg=self.fg_color, fg='white', **btn_cfg).pack(side='left', padx=10)

        # Contributions table
        self.tree = ttk.Treeview(self.scrollable_frame, columns=["ID", "Name", "Type", "Amount", "Payment", "Date", "Status"], show="headings")
        ...
        # Right side content - Credits and Totals
        tk.Label(self.right_frame, text="Credits Management", font=self.title_font,
                 bg=self.bg_color, fg=self.fg_color).pack(pady=10, anchor='w')

        # Credits form
        credits_form = tk.LabelFrame(self.right_frame, text="Add Credit", font=self.label_font,
                                    bg=self.bg_color, fg=self.fg_color)
        ...
        tk.Button(credits_form, text="Add Credit", command=self.add_credit, bg=self.fg_color, fg='white',
                 font=self.button_font).grid(row=4, column=0, columnspan=2, pady=10)

        # Credits table
        credits_table_frame = tk.LabelFrame(self.right_frame, text="Credits", font=self.label_font,
                                          bg=self.bg_color, fg=self.fg_color)
        credits_table_frame.pack(fill='x', padx=5, pady=5)

        self.credits_tree = ttk.Treeview(credits_table_frame, columns=["ID", "Name", "Amount", "Date", "Reason"], show="headings", height=5)
        ...
        total_frame = tk.LabelFrame(self.right_frame, text="Total Contributions", font=self.label_font,
                                   bg=self.bg_color, fg=self.fg_color)
        total_frame.pack(fill='x', padx=5, pady=10)

        self.total_label = tk.Label(total_frame, text="Total: ₹0", font=('Arial', 14, 'bold'), bg=self.bg_color)
        self.total_label.pack(pady=10)

        tk.Button(total_frame, text="Show Total", command=self.show_total_contributions, bg=self.fg_color, fg='white',
    ...
    def show_total_contributions(self):
        try:
            ...
            # Calculate net total
            net_total = float(total_contributions) - float(total_credits)
            
            self.total_label.config(text=f"Total: ₹{net_total:,.2f}\n"
                                       f"(Contributions: ₹{total_contributions:,.2f}\n"
                                       f"Credits: ₹{total_credits:,.2f})")
        except Exception as e:
            messagebox.showerror("Error", f"Failed to calculate totals: {str(e)}")
