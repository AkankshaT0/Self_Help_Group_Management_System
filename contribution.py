...
class ContributionManagement:
    ...
    def create_ui(self):
        ...
        tk.Label(self.right_scrollable_frame, text="Debits Management", 
                font=self.title_font, bg=self.bg_color, fg=self.fg_color).pack(pady=10, anchor='w')

        # Credits form
        credits_form = tk.LabelFrame(self.right_scrollable_frame, text="Add/Edit Debit", 
                                   font=self.label_font, bg=self.bg_color, fg=self.fg_color)
        ...
        # Credits table
        credits_table_frame = tk.LabelFrame(self.right_scrollable_frame, text="Debits", 
                                          font=self.label_font, bg=self.bg_color, fg=self.fg_color)
        ...
        member_credits_frame = tk.LabelFrame(self.right_scrollable_frame, text="Member Debits", 
                                           font=self.label_font, bg=self.bg_color, fg=self.fg_color)
        ...
        # Net total section
        total_frame = tk.LabelFrame(self.right_scrollable_frame, text="Net Total (Contributions - Debits)", 
                                   font=self.label_font, bg=self.bg_color, fg=self.fg_color)
    ...
    def add_credit(self):
        try:
            ...
            messagebox.showinfo("Success", "Debit added successfully")
        except Exception as e:
            messagebox.showerror("Error", str(e))
    ...
    def update_credit(self):
        selected = self.credits_tree.selection()
        if not selected:
            messagebox.showwarning("Select Record", "Please select a debit to update")
            return
    ...
    def delete_credit(self):
        selected = self.credits_tree.selection()
        if not selected:
            messagebox.showwarning("Select Record", "Please select a debit to delete")
            return
